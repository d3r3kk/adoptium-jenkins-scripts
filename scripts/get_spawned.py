...
class JenkinsConsoleParser:
    ...
    def _extract_spawned_jobs(self, lines: List[str]) -> List[Dict[str, Any]]:
        ...
        # Convert dict to list and filter out jobs with unknown build numbers if better entries exist
        spawned_jobs = []
        processed_names = set()
        ...
        # Second pass: add jobs with unknown build numbers only if no better entry exists
        for job_key, job_info in jobs_dict.items():
...
Examples:
  ...
  %(prog)s --input /path/to/console.txt --output /path/to/output.json
    ...
    try:
        ...
        result = parser.parse_console_output(console_content)
