...
def get_console_log(jenkins_url, username, api_token, pipeline_name, run_number):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("Error: Authentication failed. Please check your username and API token.", file=sys.stderr)
            sys.exit(1)
        elif response.status_code == 404:
            print(f"Error: Pipeline run not found. Please check the pipeline name '{pipeline_name}' and run number '{run_number}'.", file=sys.stderr)
            sys.exit(1)
        else:
            print(f"Error: Failed to retrieve console log. HTTP status code: {response.status_code}", file=sys.stderr)
            print(f"Response: {response.text}", file=sys.stderr)
            sys.exit(1)
            
    except requests.exceptions.ConnectionError:
        print(f"Error: Unable to connect to Jenkins server at '{jenkins_url}'. Please check the URL.", file=sys.stderr)
        sys.exit(1)
    except requests.exceptions.Timeout:
        print("Error: Request timed out. The Jenkins server may be slow to respond.", file=sys.stderr)
        sys.exit(1)
...
Examples:
  ...
  %(prog)s --token "your-api-token" --username jenkins-user --url https://ci.adoptium.net/ --pipeline-name "release-openjdk21-pipeline" --run-number 48 --output console.log
    ...
    parser.add_argument(
        '--username',
        default='anonymous',
        help='Jenkins username (default: anonymous)'
    )
    ...
    parser.add_argument(
        '--output',
        required=True,
        help='Output file path/name for console log'
    )
